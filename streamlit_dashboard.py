...
if 'Sale' in full_df.columns:
    sales_by_branch = full_df.groupby('Branch', as_index=False)['Sale'].sum()
    fig_sales = px.bar(sales_by_branch, x='Branch', y='Sale', title="Total Sales by Branch")
    st.plotly_chart(fig_sales, use_container_width=True)

# Monthly Profit Trend (all branches)
if 'Month' in full_df.columns and 'Profit' in full_df.columns:
    fig_profit = px.line(full_df, x='Month', y='Profit', color='Branch', title="Monthly Profit by Branch")
    st.plotly_chart(fig_profit, use_container_width=True)

st.info("Add more charts or KPIs as needed!")
